...
import network.impl.tp2p.Tp2pPeer;
...
//		}
	...
	public static Peer createDefaultAndStartPeerForTest() {
		...
		return createDefaultAndStartPeerForTest(Application.tomp2pPort, Application.rdvPeerIds);
	}
	...
	public static Peer createDefaultAndStartPeerForTest(int port, String[] rdvPeerIds) {
		...
		System.out.println("tomp2p will run on port " + port);
		Peer p = createAndStartPeer("tomp2p", cache, port, rdvPeerIds);
		return p;
	}
	...
	public static Peer createAndStartPeer(String impl, String tmpFolder, int port, String[] rdvPeerIds){
		...
		case "tomp2p": peer = createTmp2pPeer(port); break;
	...
	public static Peer createTmp2pPeer(){ return createTmp2pPeer(Application.tomp2pPort); }
	public static Peer createTmp2pPeer(int port) {
		Logger.getLogger("net.tomp2p").setLevel(Level.SEVERE);
		AdvertisementBridge i = new AdvertisementBridge();
		AdvertisementFactory.registerAdvertisementInstance(i.getAdvType(), new AdvertisementInstanciator(i));
		return new Tp2pPeer(port);
	}
}
